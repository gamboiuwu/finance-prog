    ...
    <div className="px-4 space-y-4">
      ...
      <div className="bg-slate-900 rounded-xl p-4 space-y-2.5">
        ...
        <div>
          <button onClick={() => setShowOpex(v => !v)} aria-expanded={showOpex} className="w-full flex justify-between text-sm">
            <span className="text-slate-400">− Operating expenses {t.opexByCat.length > 0 && <span aria-hidden="true" className="text-slate-600">{showOpex ? '▾' : '▸'}</span>}</span>
        ...
        <div>
          <button onClick={() => setShowSubs(v => !v)} aria-expanded={showSubs} className="w-full flex justify-between text-sm">
            <span className="text-slate-400">− Deductible subscriptions {t.dedSubs.length > 0 && <span aria-hidden="true" className="text-slate-600">{showSubs ? '▾' : '▸'}</span>}</span>
      ...
      <div className="bg-slate-900 rounded-xl p-4">
        <button onClick={() => setShowPersonal(v => !v)} aria-expanded={showPersonal} className="w-full flex justify-between items-baseline">
          <span className="text-slate-500 text-[10px] uppercase tracking-widest">Personal Income {year} {t.personalBySource.length > 0 && <span aria-hidden="true" className="text-slate-600">{showPersonal ? '▾' : '▸'}</span>}</span>
