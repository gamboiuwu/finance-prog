...
| 21 | Budget Category Notes & Annotations | ✅ COMPLETED + VERIFIED + DOUBLE-CHECKED (2026-06-03) |
...
| 24 | Spending Calendar Heatmap | ✅ COMPLETED + VERIFIED (2026-06-03) |
...
- **Task 24**: ✅ COMPLETED + VERIFIED (2026-06-03). `SpendingHeatmap` component in Budget.jsx (after `AllEntriesView`, before `Sparkline`). Placement = **Budget → Entries & Trends tab** (per user answer "Budget → Entries"), inserted between `TrendsView` and `AllEntriesView` (Budget.jsx ~line 1507). Collapsible card "📅 Spending Calendar" (default collapsed). 7-col grid; leading blanks via `firstDow`; day cells graded by `tier(mag)` = day's share of month-max (4 tiers, relative — avoids hard-coded $ threshold). Spend-dominant days use `HEAT_ROSE`, income/deposit-dominant days use `HEAT_TEAL` (user wanted income days teal); days with BOTH get a small corner dot in the opposite colour. Today gets a thin slate outline; selected day a sky-blue outline. Month nav ‹ › arrows (forward disabled at current month); default = current month. Tap a day → inline detail panel below grid listing each txn (type · desc · signed amount) + day spent/in totals. Legend row (rose gradient + teal income swatch). Reuses `allAllocTx` (already loaded) — zero new API calls, no new localStorage (view state in-memory). Build passes.
